...
- **Status**: ✅ Phase 1 완료·종료 (2026-06-03) — 56 테스트 통과, 실데이터 검증 완료. 후속은 Phase 2 백로그 참조.
...
## Phase 1 종료 (2026-06-03)

여기서 Phase 1을 마무리한다. 달성한 것:

- ✅ 모듈 구현 완료 (indicators / data / strategy / backtest engine·costs·metrics / run CLI)
- ✅ 단위 테스트 56개 통과 (합성 데이터로 비용·체결·손익 산수 검증)
- ✅ 실데이터 백테스트 파이프라인 동작 (FDR 라이브)
- ✅ **핵심 결론 확보**: baseline 평균회귀는 비용 차감 후 엣지가 미약하고, 꼬리 위험이 실측됨
  → "돈을 넣기 전에 코드로 먼저 검증"이라는 Phase 1 목적 달성

**Phase 1의 가치는 "전략이 통한다"가 아니라 "이 baseline은 안 통한다를 싸게 확인했다"이다.**

## Phase 2 백로그 (다음에 할 일)

엣지 발굴/검증을 이어가려면:

- [ ] **데이터 확장**: FDR 3000봉 제한(2014~) → 2002년부터 전체 기간 확보(2008 금융위기 포함 꼬리 검증)
- [ ] **신호 보강**: V-KOSPI(공포지수)·거래량 급증 조건 추가
- [ ] **리스크**: 손절·포지션 사이징(고정비율→켈리/변동성 기반) 도입
- [ ] **파라미터 탐색**: RSI 임계·청산일수 등 워크포워드로 (과최적화 주의)
- [ ] **가설 재검토**: 위로도 안 되면 평균회귀 가설 자체를 재고
- [ ] 인트라데이 확장 시 실시간 분봉 수집기 별도 계획
- [ ] 개별 종목 확장 시 생존편향(상폐 포함) 처리
- [ ] 수정주가(배당/분할) 반영 여부 데이터 소스에서 확인
