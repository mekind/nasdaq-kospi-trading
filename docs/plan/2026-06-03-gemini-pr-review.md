...
2. **다중 역할(persona) + 스티키 코멘트**: 버그·정확성(🐛), 보안(🔒), 성능(⚡), 가독성·유지보수(🧹) 4개 역할이 각각 Gemini 호출 → **역할별로 별도 코멘트**를 단다. 각 역할은 HTML 마커(`<!-- gemini-review:<role> -->`)로 식별되는 **단일 스티키 코멘트**를 매 실행마다 갱신(없으면 생성) → 푸시마다 코멘트가 누적되지 않는다.
   - 트리거: `opened`, `synchronize`, `reopened`.
   - 오탐 억제 규칙(공통): diff에 보이는 것만, `gh`/표준도구 기본설치 가정, 커밋 날짜 지적 금지, 확신 없으면 생략, 문제 없으면 `LGTM`.
   - 게시는 스크립트가 GitHub REST API로 직접 수행(역할별 다중 코멘트 처리 용이).
